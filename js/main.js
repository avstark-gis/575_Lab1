...
function createMap() {
    ...
    let map = L.map('map', {
        ...
        zoom: 4,
        minZoom: 4,
        layers:[mapboxMap, overlayCities]
    });
...
function getData(map) {
    ...
    $.ajax("data/temp_100avg.geojson", {
        ...
        success: function(response){
            ...
            /* //another set of data for overlay
            $.ajax("data/temp2021.geojson",   {
                dataType: "json",
                success: function (response){
                    var attributes = processData(response);
                
                    newOverlay(response, map, attributes);
